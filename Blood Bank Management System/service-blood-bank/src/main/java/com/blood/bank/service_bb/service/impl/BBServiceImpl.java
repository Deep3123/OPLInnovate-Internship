...
public class BBServiceImpl implements BBService {
	...
	public String saveDonatedBloodGroupDetails(BloodGroupAvailbilityStatusProxy availbilityStatusProxy) {
		...
		BloodGroupAvailbilityStatus status = MapperUtile.convertValue(availbilityStatusProxy,
				BloodGroupAvailbilityStatus.class);

		// Fetch the existing BloodGroupAvailbilityStatus by name
		Optional<BloodGroupAvailbilityStatus> existingStatusOpt = availbilityStatusRepository
				.findByBloodGroupName(status.getBloodGroupName());
		...
		if (existingStatusOpt.isPresent()) {
			...
			existingDonationDetails.add(status.getDonationDetails().get(0)); // Add the first donation from the new
																				// status

			// Update the available blood units
			int updatedBloodUnits = existingStatus.getAvailabBloodInUnit()
					+ status.getDonationDetails().get(0).getDonatedBloodUnit();
	...
	public List<BloodDonationDetailsProxy> getAllDonationDetails(String donorUsername) {
		List<BloodDonationDetails> list = bloodDonationDetailsRepository.findByDonorUserName(donorUsername)
				.orElseThrow(() -> new RuntimeException("Details Not found with given input !!"));
	...
	public BloodGroupAvailbilityStatusProxy getBloodGroupDetails(String bloodGroupName) {
		BloodGroupAvailbilityStatus availbilityStatus = availbilityStatusRepository.findByBloodGroupName(bloodGroupName)
				.orElseThrow(() -> new RuntimeException("BloodGroup Not found with given input !!"));
	...
	public String saveOrderDetails(PatientBloodUtilizationHistoryProxy bloodUtilizationHistoryProxy) {
		PatientBloodUtilizationHistory value = MapperUtile.convertValue(bloodUtilizationHistoryProxy,
				PatientBloodUtilizationHistory.class);
